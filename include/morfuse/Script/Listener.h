...
namespace mfuse
{
	...
	public:
		...
		mfuse_EXPORTS void PostEvent(Event *ev, uinttime_t delay, int flags = 0);
		mfuse_EXPORTS bool PostponeAllEvents(uinttime_t time);
		mfuse_EXPORTS bool PostponeEvent(Event& ev, uinttime_t time);
		...
		mfuse_EXPORTS bool ProcessScriptEvent(Event &ev);
		mfuse_EXPORTS bool ProcessScriptEvent(Event *ev);
		...
		mfuse_EXPORTS bool BroadcastEvent(const rawchar_t* name, Event& event);
		mfuse_EXPORTS bool BroadcastEvent(const_str name, Event& event);
		mfuse_EXPORTS void CancelWaiting(const rawchar_t* name);
		mfuse_EXPORTS void CancelWaiting(const_str name);
		mfuse_EXPORTS void CancelWaitingAll(void);
		mfuse_EXPORTS void CancelWaitingSources(const_str name, ConList& listeners, ConList& stoppedListeners);

		mfuse_EXPORTS void ExecuteThread(const rawchar_t* scriptName, const rawchar_t* label, Event *params = NULL);
		mfuse_EXPORTS void ExecuteThread(const rawchar_t* scriptName, const rawchar_t* label, Event &params);

		mfuse_EXPORTS void EndOn(const rawchar_t* name, Listener *listener);
		mfuse_EXPORTS void EndOn(const_str name, Listener *listener);
		mfuse_EXPORTS void Notify(const rawchar_t *name);
		mfuse_EXPORTS void Register(const rawchar_t* name, Listener *listener);
		...
		mfuse_EXPORTS void Unregister(const rawchar_t* name);
		mfuse_EXPORTS void Unregister(const_str name);
		mfuse_EXPORTS void Unregister(const rawchar_t* name, Listener *listener);
		mfuse_EXPORTS void Unregister(const_str name, Listener *listener);
		mfuse_EXPORTS void UnregisterAll(void);
		mfuse_EXPORTS bool UnregisterSource(const_str name, Listener *listener);
		mfuse_EXPORTS bool UnregisterTarget(const_str name, Listener *listener);
		mfuse_EXPORTS void UnregisterTargets(const_str name, ConList &listeners, ConList &stoppedListeners, con::Container<const_str> &stoppedNames);
		mfuse_EXPORTS void AbortRegistration(const_str name, Listener *l);

		mfuse_EXPORTS size_t RegisterSize(const_str name) const;
		mfuse_EXPORTS size_t RegisterSize(const rawchar_t* name) const;
		mfuse_EXPORTS size_t WaitingSize(const_str name) const;
		mfuse_EXPORTS size_t WaitingSize(const rawchar_t* name) const;

		mfuse_EXPORTS bool WaitTillDisabled(const rawchar_t* s);
	...
	private:
		...
		void PostEventInternal(Event* ev, uinttime_t delay, int flags);
